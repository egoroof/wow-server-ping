...
- `T1` - таймауты во время изначального TCP соединения (`Conn`)
- `T2` - таймауты после `T1` и до получения первого сообщения от сервера (`Hand`)
- `T3` - таймауты после `T2`, во время отправки сообщения серверу и до получения ответа от сервера (`Ping`)
- `E` - ошибки
...
Время подключения (`Conn`) измеряется в шагах 1 - 2, время установления соединения (`Hand`) измеряется в шагах 3 - 7, а пинг (`Ping`) сервера в шагах 8 - 9.
