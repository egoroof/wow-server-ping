...
Списки миров некоторых популярных серверов уже включены в сборку:

- WoW Circle
- Warmane
- EZ WoW

Если вас интересует эти сервера, то список миров не нужно извлекать. Можете переходить к следующий главе.

Это утилита не работает с сервером Sirus.
...
Запустите `realmlist.bat` на Windows (из папки `bat`) или `realmlist` на Linux. Нужно будет ввести адрес сервера, ваш логин и пароль.
...
| `-ping-auth` | `false` | Следует ли пинговать сервера аутентификации |
